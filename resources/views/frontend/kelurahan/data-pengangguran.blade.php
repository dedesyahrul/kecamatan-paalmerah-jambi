...
@section('title',
    'Data Warga Pengangguran Kelurahan ' .
    $kelurahan->nama .
            ...
            <div class="row">
                ...
                <div class="col-lg-8 col-md-6">
                    <div class="card blog blog-detail border-0 shadow rounded">
                        <div class="card-body">
                            <h3 class="card-title">{{ $dataPengangguran->judul }}</h3>
                            <ul class="list-unstyled d-flex justify-content-between mt-4">
                                <li class="list-inline-item date text-muted"><i class="uil uil-calendar-alt text-dark"></i>
                                    {{ \Carbon\Carbon::parse($dataPengangguran->created_at)->translatedFormat('l, d F Y') }}
                                </li>
                            </ul>
                            <hr>
                            <div>
                                @if ($dataPengangguran->foto && Storage::disk('public')->exists($dataPengangguran->foto))
                                    <img src="{{ asset('storage/' . $dataPengangguran->foto) }}"
                                        class="img-fluid rounded-md shadow mb-4" alt="Foto Pengangguran">
                                @endif
                                <small>{{ $dataPengangguran->text_foto }}</small>
                                <div class="tinymce-content">
                                    {!! $dataPengangguran->isi !!}
                                </div>
                            </div>
                        </div>
                    </div>
