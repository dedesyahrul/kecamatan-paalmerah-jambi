            ...
            <div class="row">
                ...
                <div class="col-lg-8 col-md-6">
                    <div class="card blog blog-detail border-0 shadow rounded">
                        <div class="card-body">
                            <h3 class="card-title">{{ $dataPbb->judul }}</h3>
                            <ul class="list-unstyled d-flex justify-content-between mt-4">
                                <li class="list-inline-item date text-muted"><i class="uil uil-calendar-alt text-dark"></i>
                                    {{ \Carbon\Carbon::parse($dataPbb->created_at)->translatedFormat('l, d F Y') }}
                                </li>
                            </ul>
                            <hr>
                            <div>
                                @if ($dataPbb->foto && Storage::disk('public')->exists($dataPbb->foto))
                                    <img src="{{ asset('storage/' . $dataPbb->foto) }}"
                                        class="img-fluid rounded-md shadow mb-4" alt="Foto PBB">
                                @endif
                                <small>{{ $dataPbb->text_foto }}</small>
