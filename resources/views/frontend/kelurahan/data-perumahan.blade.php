            ...
            <div class="row">
                ...
                <div class="col-lg-8 col-md-6">
                    <div class="card blog blog-detail border-0 shadow rounded">
                        <div class="card-body">
                            <h3 class="card-title">{{ $dataPerumahan->judul }}</h3>
                            <ul class="list-unstyled d-flex justify-content-between mt-4">
                                <li class="list-inline-item date text-muted"><i class="uil uil-calendar-alt text-dark"></i>
                                    {{ \Carbon\Carbon::parse($dataPerumahan->created_at)->translatedFormat('l, d F Y') }}
                                </li>
                            </ul>
                            <hr>
                            <div>
                                @if ($dataPerumahan->foto && Storage::disk('public')->exists($dataPerumahan->foto))
                                    <img src="{{ asset('storage/' . $dataPerumahan->foto) }}"
                                        class="img-fluid rounded-md shadow mb-4" alt="Foto Perumahan">
                                @endif
                                <small>{{ $dataPerumahan->text_foto }}</small>
